...
export class WorkspaceSvg extends Workspace {
    ...
    getTopComments(ordered = false) {
        return super.getTopComments(ordered);
    }
    /**
     ...
     */
    getTopBoundedElements(ordered = false) {
        const elements = new Array().concat(this.topBoundedElements);
        if (ordered) {
    ...
    getNestedTrees() {
        const nestedWorkspaces = this.getAllBlocks()
            ...
            .flat()
            .filter((icon) => icon instanceof MutatorIcon && icon.bubbleIsVisible())
    ...
    lookUpFocusableNode(id) {
        ...
        const commentIdSeparatorIndex = Math.max(id.indexOf(COMMENT_EDITOR_FOCUS_IDENTIFIER), id.indexOf(COMMENT_COLLAPSE_BAR_BUTTON_FOCUS_IDENTIFIER), id.indexOf(COMMENT_DELETE_BAR_BUTTON_FOCUS_IDENTIFIER));
