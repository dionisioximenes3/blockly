...
export class FocusManager {
    ...
    registerTree(tree, rootShouldBeAutoTabbable = false) {
        ...
        this.registeredTrees.push(new TreeRegistration(tree, rootShouldBeAutoTabbable));
        const rootElement = tree.getRootFocusableNode().getFocusableElement();
        if (!rootElement.id || rootElement.id === 'null') {
            throw Error(`Attempting to register a tree with a root element that has an ` +
                `invalid ID: ${tree}.`);
        }
    ...
    unregisterTree(tree) {
        ...
        const treeIndex = this.registeredTrees.findIndex((reg) => reg.tree === tree);
    ...
    focusNode(focusableNode) {
        ...
        const matchedNode = FocusableTreeTraverser.findFocusableNodeFor(focusableNodeElement, nextTree);
     ...
     *     should end.
     */
    takeEphemeralFocus(focusableElement, onFocusChangedInDom = null) {
        this.ensureManagerIsUnlocked();
        if (this.ephemerallyFocusedElement) {
        ...
        return () => {
            ...
            this.ephemerallyFocusedElement = null;
            this.ephemeralDomFocusChangedCallback = null;
            const hadEphemeralFocusAtEnd = this.ephemerallyFocusedElementCurrentlyHasFocus;
            this.ephemerallyFocusedElementCurrentlyHasFocus = false;
